 ...
 * @category	XML-RPC
 * @author		ExpressionEngine Dev Team
 * @link		http://itfosters.com/user_guide/libraries/xmlrpc.html
 */
class ITF_Xmlrpc {
	...
	var $xmlrpcName			= 'XML-RPC for ITFosters';
 ...
 * @link		http://itfosters.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Client extends ITF_Xmlrpc
{
	var $path			= '';
	var $server			= '';
	var $port			= 80;
	...
	function decode($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = ($this->xss_clean) ? $ITF->security->xss_clean($array[$key]) : $array[$key];
				}
			}
		...
		else
		{
			...
			{
				$result = ($this->xss_clean) ? $ITF->security->xss_clean($result) : $result;
			}
		}
 ...
 * @link		http://itfosters.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Message extends ITF_Xmlrpc
{
	var $payload;
	var $method_name;
	var $params			= array();
	...
	function output_parameters($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				else
				{
					...
					$array[$key] = ($key == 'bits' OR $this->xss_clean == FALSE) ? $array[$key] : $ITF->security->xss_clean($array[$key]);
				}
			}
		...
		else
		{
			...
			for ($i = 0; $i < count($this->params); $i++)
			{
				...
				{
					$parameters[] = ($this->xss_clean) ? $ITF->security->xss_clean($a_param) : $a_param;
				}
			}
		}
 ...
 * @link		http://itfosters.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Values extends ITF_Xmlrpc
{
	var $me		= array();
	var $mytype	= 0;
