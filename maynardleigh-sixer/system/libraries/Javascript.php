 ...
 * @category	Javascript
 * @author		ExpressionEngine Dev Team
 * @link		http://itfosters.com/user_guide/libraries/javascript.html
 */
class ITF_Javascript {
	...
	public function __construct($params = array())
	{
		...
		$this->ITF->load->library('javascript/'.$js_library_driver, array('autoload' => $autoload));
		// make js to refer to current library
		$this->js =& $this->ITF->$js_library_driver;
	...
	function external($external_file = '', $relative = FALSE)
	{
		...
		{
			if ($this->ITF->config->item('javascript_location') != '')
			{
				$this->_javascript_location = $this->ITF->config->item('javascript_location');
			}
		}
		...
		{
			$str = $this->_open_script($this->ITF->config->slash_item('base_url').$this->_javascript_location.$external_file);
		}
	...
	{
		$str = '<script type="text/javascript" charset="'.strtolower($this->ITF->config->item('charset')).'"';
		$str .= ($src == '') ? '>' : ' src="'.$src.'">';
		return $str;
	}
