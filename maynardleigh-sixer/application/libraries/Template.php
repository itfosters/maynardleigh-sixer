 ...
 * Build your ITFosters pages much easier with partials, breadcrumbs, layouts and themes
...

{
...

	{
...

		{
			...
			class_exists('ITF_Parser') OR $this->_itf->load->library('parser');
		...
		if (method_exists( $this->_itf->router, 'fetch_module' ))
...

		{
			...
			$this->_module 	= $this->_itf->router->fetch_module();
		...
		$this->_controller	= $this->_itf->router->fetch_class();



		$this->_method 		= $this->_itf->router->fetch_method();
		...
		class_exists('ITF_User_agent') OR $this->_itf->load->library('user_agent');



		//$this->_itf->load->library('user_agent');
		...
		$this->_is_mobile	= $this->_itf->agent->is_mobile();
...

	{
		...
		$template['jscss_head']	= implode("\n\t\t", isset($this->_jscss["T"])?$this->_jscss["T"]:array());

		

		$template['jscss_footer']	= implode("\n\t\t", isset($this->_jscss["B"])?$this->_jscss["B"]:array());
		...
		foreach ($this->_partials as $name => $partials)
...

		{
			...
			foreach($partials as $partial) {	
...

				{
					...
					if(!isset($template['partials'][$name])) $template['partials'][$name]="";
...

				{
...

					{
						...
						$partial['string'] = $this->_itf->parser->parse_string($partial['string'], $this->_data + $partial['data'], TRUE, TRUE);
		...
		$this->_itf->output->set_header('Expires: Sat, 01 Jan 2000 00:00:01 GMT');



		$this->_itf->output->set_header('Cache-Control: no-store, no-cache, must-revalidate');



		$this->_itf->output->set_header('Cache-Control: post-check=0, pre-check=0, max-age=0');



		$this->_itf->output->set_header('Last-Modified: ' . gmdate( 'D, d M Y H:i:s' ) . ' GMT' );



		$this->_itf->output->set_header('Pragma: no-cache');



		// Let ITF do the caching instead of the browser



		$this->_itf->output->cache($this->cache_lifetime);
...

		{
			...
			$this->_itf->output->set_output($this->_body);
   ...
   function add_js($script, $possition="B", $type = 'import', $defer = FALSE)
...

   {
...

      {
         ...
         case 'import':
			...
			$this->_jscss[$possition][$script] = $js;
		...
		case 'external':
         ...
         case 'embed':
			...
			$this->_jscss[$possition]["JS"]= isset($this->_jscss[$possition]["JS"])?$this->_jscss[$possition]["JS"]:"";



			$this->_jscss[$possition]["JS"] .= $js;
   ...
   function add_css($style,  $possition="T", $type = 'link', $media = FALSE)
...

   {
...

      {
         ...
         case 'link':
			...
			$this->_jscss[$possition][$style] = $css;
		...
		case 'external':
         ...
         case 'embed':
			...
			$this->_jscss[$possition]["styles"] = isset($this->_jscss[$possition]["styles"])?$this->_jscss[$possition]["styles"]:"";



			$this->_jscss[$possition]["styles"] .= $css;
...

	{
		...
		if ($this->_itf->load->get_var('template_views'))
...

		{
			...
			return $this->_itf->load->get_var('template_views');
		...
		$this->_itf->load->vars('template_views', $view_folder);
...

	{
...

		{
...

			{
				...
				$content = $this->_itf->parser->parse_string($this->_itf->load->file(
...

			{
				...
				// Load it directly, bypassing $this->load->view() as ME resets _itf_view



				$content = $this->_itf->load->file(
...

		{
			...
			$content = ($this->_parser_enabled === TRUE AND $parse_view === TRUE)
				...
				? $this->_itf->parser->parse($view, $data, TRUE)
				...
				: $this->_itf->load->view($view, $data, TRUE);
...

	{
		...
		$this->_itf->load->helper('inflector');
