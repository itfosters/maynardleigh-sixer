...
define('ITF_READERS_BIFF8',			 0x600);
define('ITF_READERS_BIFF7',			 0x500);
define('ITF_READERS_WORKBOOKGLOBALS',   0x5);
define('ITF_READERS_WORKSHEET',		 0x10);
define('ITF_READERS_TYPE_BOF',		  0x809);
define('ITF_READERS_TYPE_EOF',		  0x0a);
define('ITF_READERS_TYPE_BOUNDSHEET',   0x85);
define('ITF_READERS_TYPE_DIMENSION',	0x200);
define('ITF_READERS_TYPE_ROW',		  0x208);
define('ITF_READERS_TYPE_DBCELL',	   0xd7);
define('ITF_READERS_TYPE_FILEPASS',	 0x2f);
define('ITF_READERS_TYPE_NOTE',		 0x1c);
define('ITF_READERS_TYPE_TXO',		  0x1b6);
define('ITF_READERS_TYPE_RK',		   0x7e);
define('ITF_READERS_TYPE_RK2',		  0x27e);
define('ITF_READERS_TYPE_MULRK',		0xbd);
define('ITF_READERS_TYPE_MULBLANK',	 0xbe);
define('ITF_READERS_TYPE_INDEX',		0x20b);
define('ITF_READERS_TYPE_SST',		  0xfc);
define('ITF_READERS_TYPE_EXTSST',	   0xff);
define('ITF_READERS_TYPE_CONTINUE',	 0x3c);
define('ITF_READERS_TYPE_LABEL',		0x204);
define('ITF_READERS_TYPE_LABELSST',	 0xfd);
define('ITF_READERS_TYPE_NUMBER',	   0x203);
define('ITF_READERS_TYPE_NAME',		 0x18);
define('ITF_READERS_TYPE_ARRAY',		0x221);
define('ITF_READERS_TYPE_STRING',	   0x207);
define('ITF_READERS_TYPE_FORMULA',	  0x406);
define('ITF_READERS_TYPE_FORMULA2',	 0x6);
define('ITF_READERS_TYPE_FORMAT',	   0x41e);
define('ITF_READERS_TYPE_XF',		   0xe0);
define('ITF_READERS_TYPE_BOOLERR',	  0x205);
define('ITF_READERS_TYPE_FONT',	  0x0031);
define('ITF_READERS_TYPE_PALETTE',	  0x0092);
define('ITF_READERS_TYPE_UNKNOWN',	  0xffff);
define('ITF_READERS_TYPE_NINETEENFOUR', 0x22);
define('ITF_READERS_TYPE_MERGEDCELLS',  0xE5);
define('ITF_READERS_UTCOFFSETDAYS' ,	25569);
define('ITF_READERS_UTCOFFSETDAYS1904', 24107);
define('ITF_READERS_MSINADAY',		  86400);
define('ITF_READERS_TYPE_HYPER',	     0x01b8);
define('ITF_READERS_TYPE_COLINFO',	     0x7d);
define('ITF_READERS_TYPE_DEFCOLWIDTH',  0x55);
define('ITF_READERS_TYPE_STANDARDWIDTH', 0x99);
define('ITF_READERS_DEF_NUM_FORMAT',	"%s");


/*
* Main Class
*/
class ItfExcelReader {
	...
	function infodata($sheets=0){

		$keydata = array();
		$alldata = isset($this->sheets[$sheets])?$this->sheets[$sheets]:array();

		
		foreach($alldata["cells"] as $kk=>$info){
			if($kk==1)
			{
				$keydata=$info;
			}else{
				$infodata=array();
				foreach($keydata as $k=>$d){
					$infodata[$d]=isset($info[$k])?$info[$k]:"";
				}
				$this->allinformation[]=$infodata;
			}
		}
	...
	var $_defaultFormat = ITF_READERS_DEF_NUM_FORMAT;
	 ...
	 */
	function load($file='',$store_extended_info=true,$outputEncoding='') {
		$newOLE = new OLERead();
		$this->_ole =$newOLE;
	...
	function _parse() {
		...
		if (($version != ITF_READERS_BIFF8) &&
			($version != ITF_READERS_BIFF7)) {
			return false;
		}

		if ($substreamType != ITF_READERS_WORKBOOKGLOBALS){
			return false;
		}
		...
		while ($code != ITF_READERS_TYPE_EOF) {
			switch ($code) {
				case ITF_READERS_TYPE_SST:
					$spos = $pos + 4;
					...
					break;
				case ITF_READERS_TYPE_FILEPASS:
					return false;
					break;
				case ITF_READERS_TYPE_NAME:
					break;
				case ITF_READERS_TYPE_FORMAT:
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
						...
						$this->xfRecords[] = $xf;
					break;
				case ITF_READERS_TYPE_NINETEENFOUR:
					$this->nineteenFour = (ord($data[$pos+4]) == 1);
					break;
				case ITF_READERS_TYPE_BOUNDSHEET:
						...
						}elseif ($version == ITF_READERS_BIFF7){
	...
	function _parsesheet($spos) {
		...
		if (($version != ITF_READERS_BIFF8) && ($version != ITF_READERS_BIFF7)) {
			return -1;
		}

		if ($substreamType != ITF_READERS_WORKSHEET){
			return -2;
		}
		$spos += $length + 4;
		while($cont) {
			$lowcode = ord($data[$spos]);
			if ($lowcode == ITF_READERS_TYPE_EOF) break;
			...
			unset($this->rectype);
			switch ($code) {
				case ITF_READERS_TYPE_DIMENSION:
					if (!isset($this->numRows)) {
						if (($length == 10) ||  ($version == ITF_READERS_BIFF7)){
					...
					break;
				case ITF_READERS_TYPE_MERGEDCELLS:
					...
					break;
				case ITF_READERS_TYPE_RK:
				case ITF_READERS_TYPE_RK2:
					...
					break;
				case ITF_READERS_TYPE_LABELSST:
					$row		= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column	 = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
				...
				case ITF_READERS_TYPE_FORMULA:
				case ITF_READERS_TYPE_FORMULA2:
					$row	= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					...
					break;
				case ITF_READERS_TYPE_BOOLERR:
					$row	= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					$string = ord($data[$spos+6]);
					$this->addcell($row, $column, $string);
					break;
                case ITF_READERS_TYPE_STRING:
					// http://code.google.com/p/php-excel-reader/issues/detail?id=4
					if ($version == ITF_READERS_BIFF8){
				...
				case ITF_READERS_TYPE_ROW:
					...
					break;
				case ITF_READERS_TYPE_DBCELL:
					break;
				case ITF_READERS_TYPE_MULBLANK:
					$row = ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					$cols = ($length / 2) - 3;
					for ($c = 0; $c < $cols; $c++) {
					...
					break;
				case ITF_READERS_TYPE_LABEL:
					$row	= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					$this->addcell($row, $column, substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7])<<8));
					break;
				case ITF_READERS_TYPE_EOF:
					$cont = false;
					break;
				case ITF_READERS_TYPE_HYPER:
		...
		function _getCellDetails($spos,$numValue,$column) {
			...
			if ($type == 'date') {
				...
				$utcDays = floor($numValue - ($this->nineteenFour ? ITF_READERS_UTCOFFSETDAYS1904 : ITF_READERS_UTCOFFSETDAYS));
				$utcValue = ($utcDays) * ITF_READERS_MSINADAY;
				...
				$totalseconds = floor(ITF_READERS_MSINADAY * $fractionalDay);
