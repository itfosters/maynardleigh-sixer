...
class Admin extends ITFS_Controller 
{
    ...
    { //echo "@@@@<pre>";print_r($_POST);die;
                    ...
                    $alldetails = $this->orders->getOrderDetail(array("O.id"=>$orderid));
                    $ecoemail=$data['detail']->email_Id;
                    $selleremail=$data['detail']->salesemail;
                    $sellername=$data['detail']->sellername;
                    $manageremail=$data['detail']->pmemail;
                    $co_ordinatoremail=$data['detail']->email_co_ordinator;
                    $ecofirstname=$data['detail']->first_name;
                    $ecolastname=$data['detail']->last_name;
                    //$link =site_url('contract_accept/index'.'/'.$orderid);
                    $link =site_url('contract_accept/contractDocument'.'/'.$orderid);
                    //die($link);
    			
                    $allEmail = $this->orders->getSellerUserEmail($orderid);
                    $AllemailInComma  = implode(",",$allEmail);
                    //$this->email->to($AllemailInComma);
                    $AllemailInComma = $ecoemail.",".$co_ordinatoremail.",".$AllemailInComma;
                    //echo "@@@@<pre>";print_r($AllemailInComma);die;
                    $this->email->to(array($ecoemail,$co_ordinatoremail)); 
                    ...
                    $this->email->from($selleremail,urlencode($sellername));
                    $this->email->cc(array($manageremail,$selleremail,'kanika@maynardleigh.in'));
                    ...
                    $information = array(
                    			//'username'=>$ecoemail,
                    			'name'=>$ecofirstname,
                    			'CLIENT_NAME'=>$alldetails->client_name,
                    			'ORDER_ID'=>$alldetails->order_Id,
                    			'SELLER_NAME'=>$alldetails->seller_name,
                    			'MANAGER_NAME'=>$alldetails->manager_name,
                                        'link'=>$link
                                        );
