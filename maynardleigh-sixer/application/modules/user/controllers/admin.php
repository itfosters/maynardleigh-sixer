...
class Admin extends ITFS_Controller {
...

	{
		...
		$totalpage = $this->users->totalData($conditions);
		...
		$data["results"] = $this->users->findUser($conditions,$config["per_page"], $current_page);
...

	{
		...
		$data["results"] = $this->users->find($conditions,$config["per_page"], $current_page);
		...
		if(count($_POST)>0) {
			...
			$data["frm_data"]["frm_data"] = (object)$this->input->post();
				...
				if(!empty($id)) {
					...
					$this->users->updateUser($data,array("id"=>$id));
			...
			$data["frm_data"]["frm_data"] = $this->users->getUsersInfoById($id);
...

	{
		...
		if($this->input->post("itfaction") == "delete") {			
			...
			$this->users->deleteMulti($allids);

			$this->messages->flash("Users have been succesfully deleted.","success");
		...
		elseif($this->input->post("itfaction") == "publish") {
			...
			$this->users->statusupdate($allids,"1");

			$this->messages->flash("Users have been succesfully publish.","success");
		...
		elseif($this->input->post("itfaction") == "unpublish") {
			...
			$this->users->statusupdate($allids,"0");
...

	{
		...
		if(count($_POST)>0) {
			...
			$data["frm_data"]["frm_data"] = $this->users->getUsersById($id);
...

	{
            ...
            if($this->input->post("itfaction") == "status") 

                {
                    ...
                    $res = $this->users->statuchange($recid);
