...
class Users extends ITF_Model {
    ...
    protected $participantinfo = "itf_myinfoparticipantdetails";
...
//echo "###<pre>";print_r($data);die;
    ...
    public function getcallingUserForSelect($client_id)
    {
    ...
    //now for check in user map table
    public function checkExistenceInMapTableFlage($orderid,$dlvid,$userid)
    {//echo "###<pre>";print_r($emailid);die;
       $query= $this->db->select('id')
                ->from($this->mapuser)
                ->where(array('order_id'=>$orderid,'delivery_id'=>$dlvid,'user_id'=>$userid))
                ->get()->row();
                //->result();
                //echo $this->db->last_query();die;
                return $query;
    ...
    public function showAllUsersForCalling($oid=0,$delivery_id=0)
    { 
       
        $query=$this->db->select('MU.order_id,MU.delivery_id,U.id,U.name,U.email,U.contact_no')
                ->from($this->mapuser.' as MU')
                ->join($this->name.' as U','MU.user_id=U.id')
                ->where(array('order_id'=>$oid,'delivery_id'=>$delivery_id))
                //->group_by('user_id')
                ->get()
                ->result();
   ...
   //echo $this->db->last_query();die;
    ...
    public function saveParticepantMyInfo($data = array()) {
        //echo "<pre>33333";print_r($data);
        //echo "<pre>";print_r($where);die;
 
        return $this->db->save($this->name, $data);
    ...
    function getParticepantMyInfo($userid)
    {
            // $this->db->select('MF.*,PA.name as airname,M.name,M.detail',false);
            // $this->db->from($this->myinfodetail.' as MF');
            // $this->db->join($this->membership.' as M','MF.userid=M.user_id');
    ...
    //get all email address of given user id
    public function getEmailAddressById($userid = array()) {
        $this->db->select('U.email,U.name');
        $this->db->from($this->name.' U');
        $this->db->where_in('U.id',$userid);
        $query = $this->db->get()->result();
        return $query;
    }
}