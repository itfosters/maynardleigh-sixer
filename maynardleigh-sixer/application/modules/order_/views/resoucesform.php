...
CKEDITOR.replace('cani', config);
CKEDITOR.replace('why_were_we_there', config);
CKEDITOR.replace('what_was_the_solution', config);
CKEDITOR.replace('what_was_the_outcome', config);
CKEDITOR.replace('what_could_go_better', config);
CKEDITOR.replace('additional_comments', config);
CKEDITOR.replace('training_materials_given', config);
CKEDITOR.replace('books_name_and_quantity', config);
CKEDITOR.replace('cards_name_and_quantity', config);
CKEDITOR.replace('any_future_business_development_opportunities', config);
</script>
...
(function(){
    ...
    $.ajax({
    url:FRONTURL+'order/resourceFormAjax',
    type:'post',
    dataType:'json',
    data:{'data':frmdata1},
