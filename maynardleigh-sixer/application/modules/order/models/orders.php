...
class Orders extends ITF_Model {
    ...
    private  $cmpType="complaintype";
    private  $complain_response="order_complain_response";
    private  $complain_action="complain_action";
    private  $complain_related="complain_related";
    ...
    public function saveComplains($array=array(), $where=array()){
        if(count($where) >0){
            $this->db->update($this->order_complain,$array, $where);
            return $where['cmp_id'];
        }else{
            $this->db->save($this->order_complain, $array);
            return $this->db->insert_id();
        }
        
    }
    public function saveResponsible($array=array(), $where=array()){
        if(count($where) >0){
            return $this->db->update($this->complain_response,$array, $where);
        }else{
            //echo "<pre>";print_r($array);die;
            return $this->db->save($this->complain_response, $array);
        }
    }
    public function deleteComplain($array=array()){
       return  $this->db->delete('itf_order_complain',$array);
    }
    //delete for all responsible
    public function deleteResponsible($wherearray=array()){
       $this->db->delete($this->complain_response,$wherearray);
    ...
    public function getComplains($ordId=0,$compid=0){
        $this->db->select("OC.*,O.order_Id");
        $this->db->from('itf_order_complain'.' as OC');
        $this->db->join($this->name.' as O','O.id=OC.orderID');
        $this->db->order_by('OC.cmp_id','desc');
        if($compid>0){
            $this->db->where(array('OC.orderID'=>$ordId,'OC.cmp_id'=>$compid));
        }else{
    ...
    public function getResponsibleName($cmpId=0,$statustype=0){
        $this->db->select("U.id,U.name");
        $this->db->from($this->complain_response.' as CR');
        $this->db->join('users'.' as U','U.id=CR.userid');
        $this->db->where(array('CR.cmpid'=>$cmpId,'status_type'=>$statustype));
        $query= $this->db->get()->result();
        //echo $this->db->last_query();die;
        return $query;
        //return $this->db->insert("itf_order_complain", $array);
    ...
    public function getResponsibleEmail($cmpId=0,$statustype=1){
        $this->db->select("CR.userid");
        $this->db->from($this->complain_response.' as CR');
        $this->db->where(array('CR.cmpid'=>$cmpId,'status_type'=>$statustype));
        $query= $this->db->get()->result();
        //echo $this->db->last_query();die;
    ...
    public function saveComplainAction($data=array()){
        return $this->db->insert($this->complain_action, $data);
    }
   
    //delete all compains action
    public function deleleComplainAction($data=array()){
        return $this->db->delete($this->complain_action, $data);
    }
    
    //save complain related to
    public function saveComplainRelatedTo($data=array()){
        return $this->db->insert($this->complain_related, $data);
    }
    //delete complain related to
    public function deleteComplainRelatedTo($array=array()){
          $ef=$this->db->delete($this->complain_related,$array);
          return $ef;
    }
    
    public function getCMPAction($cmp_id=0,$action_type=0){
        $this->db->select("CA.actId,CA.actname,CA.status");
        $this->db->from($this->complain_action. ' CA');
        $this->db->where(array('compId'=>$cmp_id,'action_type'=>$action_type));
        return $this->db->get()->result();
        //return $this->db->insert("itf_complain_action", $data);
    }
    //get complains related to
    public function getCompRelatedTo($cmp_id=0){
        $this->db->select("CR.id,concat(CR.related_id,'#',CR.order_type)as orderid",false);
        $this->db->from($this->complain_related. ' CR');
...
//echo "$$$<pre>";print_r($tableArray[$o_type]);die;
    ...
    //get Client Name by Order Id
    public function getClientNameByOrderId($ids)
    {
        $this->db->select('U.name,O.order_Id');
        $this->db->from($this->users.' U');
        $this->db->join($this->name.' O','O.client_id=U.id');
