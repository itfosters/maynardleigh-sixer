...
<?php if ((isset($frmdata['allTimeSlots'])) && (count($frmdata['allTimeSlots'])>0)) { 
    ...
    ?> 
                  ...
                  <div class="table-responsive"> 
                   <div style="padding-left: 200px;"><strong>Time Slots  List </strong></div>
<!--                   <div style="padding-left: 200px;"><strong>Time Slots list,<?php if(isset($frmdata['allTimeSlots'])){ print_r(substr($frmdata['allTimeSlots'][0]->time_slot_start,0, -8));  }   ?></strong></div>-->
                     <table class="table table-striped table-bordered table-hover" summary="">
                        ...
                        <tbody>
                           ...
                           <tr>
                              <td><?php echo ++$i;?></td>
                              <td><?php  echo date('jS F, Y h :i A',$value->timestampstart);  ?></td>
                              <td><?php  echo date('jS F, Y h :i A',$value->timestampend);                              
                              ?> &nbsp;&nbsp;&nbsp;
                            ...
                            <td><?php  echo (isset($value->name) && !empty($value->name)) ? $value->name:'';  ?></td>
                             ...
                             <td> 
                              <a type="button" value="view_details" title="Edit" href="<?php echo site_url('admin/delivery/deletetimeslot/'.$value->id."/".$frmdata['order_id']."/".$frmdata['diagnose_id']."/".$frmdata['selectedResource']);?>" onclick="return confirm('Are you sure to delete this time slot?')">
                              <i class="fa fa-trash-o"></i>
                              </a>
                              
                             </td>                        
                             <td> 
                              <a type="button" value="view_details" title="Edit Time / User" class="travel-pop" href="<?php echo site_url('admin/delivery/updateTimeSlotpop/'.$value->id."/".$frmdata['order_id']."/".$frmdata['diagnose_id']."/".$frmdata['selectedResource']."/".$value->user_id);?>">
                             ...
                             <td>
                                 ...
                                 <a data-id="<?php echo site_url('usercall_accept/index/'.$frmdata['order_id']."/".$frmdata['diagnose_id']."/".$value->user_id); ?>" title="Link for User Acceptance!"  href="#addBookDialog" class="open-AddBookDialog" data-toggle="modal" data-target="#myModal"><i class="fa fa-link"></i></a>
                                 <?php } ?>
                             </td>                        
                           </tr>
                           <?php  }?>
    ...
    <!-- Modal -->
<div id="myModal" class="modal fade" role="dialog" id="addBookDialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Global link for user time acceptance</h4>
      </div>
...
<script>
    ...
    $(document).on("click", ".open-AddBookDialog", function () {
     var myLinkValue = $(this).data('id');
     //console.log(myBookId);
     $(".modal-body #linkId").html( myLinkValue );
    });
</script>