...
class Admin extends ITFS_Controller {
    ...
    public function index($id = 0) {//die($id);
        ...
        if (isset($_POST["q"])) {
            $this->session->set_userdata(array("DELIVERYSEARCHFIELD" => $this->input->post()));
        }

        $searchfield = $this->session->userdata("DELIVERYSEARCHFIELD");
        ...
        $data['excutivecallflage'] = single_array($this->deliveries->excutiveCallProduct(), 'id', 'id');
        $data['catalystcallflage'] = single_array($this->deliveries->catalystCallProduct(), 'id', 'id');
...
//        echo "<pre>";print_r($casting_manager);
    ...
    public function trfPopUp($oid = "", $subproducts = "", $user_id = "", $id = '',$selected='') {
        //echo "<pre>##";print_r($id);die;
        
        //echo "<pre>";print_r($olddata);die;
        $data = array();
        $data['olddata'] = $this->diagnoses->getResourceInfoByOrderIDAndOrderType($oid,$subproducts,$user_id);
        $data['order_id'] = $oid;
    ...
    public function archievePopUp($oid = "", $diagnose_id = "", $type_id = '3') {
        //echo "<pre>##";print_r($id);die;
        if ($this->input->post()) {
            $modulemaster = array('1' => 'diagnose', '2' => 'design', '3' => 'delivery', '4' => 'discovery');
            $postalldata = $this->input->post();
            //echo "<pre>";print_r($postalldata);die;
            $updatedresponse = $this->diagnoses->updateInvoice($postalldata['order_id'], $postalldata['diagnose_id'], $postalldata['type_id'], $postalldata['invoice_no']);
            if ($updatedresponse)
                $this->messages->flash('Order closed successfully!');
            else
                $this->messages->flash('Failed ! Check resource is properly assinged and respective dates are correct.', 'error');
            redirect("admin/" . $modulemaster[$postalldata['type_id']] . "/index/" . $postalldata['order_id']);
        }
    ...
    public function form($id = '', $subproduct = '', $page = 0) {
        ...
        $data['calalyst_call_product'] = $array_of_call;
...
//echo "<pre>";print_r($results);die;
    ...
    public function delete() {
        ...
        $this->diagnoses->assignUpdateStatus($id, $oid, 3);
        if ($delete == 1)
    ...
    public function calender($oid = "", $dvyid = "", $resourceid = '') {
        ...
        $lunchtimegap = 29;
        $array_of_call = single_array($this->deliveries->getAllCatalystCallProducts(), 'id', 'id');
        $array_of_Excutivecall = single_array($this->deliveries->excutiveCallProduct(), 'id', 'id');
        ...
        if ($this->input->post()) {
            ...
            //echo "<pre>1234";print_r($postalldata);die;
            //Date Range
            ...
            $daydiffence = ($sessiondateend - $sessiondatestart) / (3600 * 24);
            if ($daydiffence > 0) {
                $alldaysvalue = array();
                for ($countervalue = 0; $countervalue <= $daydiffence; $countervalue++) {
                    $alldaysvalue[] = date('m/d/Y', $sessiondatestart);
                    $sessiondatestart = strtotime("+1 day", $sessiondatestart);
            ...
            //echo "<pre>".count($alldaysvalue);print_r($alldaysvalue);echo "<hr>";
            //echo "<pre>";print_r($datainfo->products);
            //echo "<pre>";print_r($alldaysvalue);die;

            if ((count(($alldaysvalue) > 0)) && (in_array($datainfo->products, $array_of_call))) {
                foreach ($alldaysvalue as $todaydate) {
                    $datatiming[0]['startdate'] = $todaydate;
                    //echo "<pre>";print_r($todaydate);
                    //echo "<pre>";print_r($datatiming);
                    //$datatiming['startdate'] = DateTime::createFromFormat('m/d/Y h:i A', $datatiming[0][0] . " " . $datatiming[0][1] . " " . $datatiming[0][2]);
                    $datatiming['startdate'] = DateTime::createFromFormat('m/d/Y h:i A', $todaydate . " " . $datatiming[0][1] . " " . $datatiming[0][2]);
                    //$datatiming['enddate'] = DateTime::createFromFormat('m/d/Y h:i A', $datatiming[1][0] . " " . $datatiming[1][1] . " " . $datatiming[1][2]);
                    $datatiming['enddate'] = DateTime::createFromFormat('m/d/Y h:i A', $todaydate . " " . $datatiming[1][1] . " " . $datatiming[1][2]);
                    //echo "<pre>555555";print_r($datatiming);die;
                    ...
                    if (in_array($datainfo->products, $array_of_call)) {
                        ...
                        while ($ii <= $totalminutesintv) {
                            ...
                            $lunchtimedata['st']->modify('+' . $lunchtimegap . ' minutes');
                            ...
                            if (($strgdata['st']->getTimestamp() <= $endtime->getTimestamp() && $strgdata['en']->getTimestamp() >= $endtime->getTimestamp() &&
                                ...
                                if (
                                    ...
                                    //getting start time
                                    $afterlunchtime = $strgdata['st']->date;
                                    $afterlunchtimestart = $datatiming['startdate'];

                                    $dteDiffafterlunch = $strgdata['st']->diff($datatiming['startdate']);
                                    $totalminutesintvafter = ($dteDiffafterlunch->days * 24 * 60) + ($dteDiffafterlunch->h * 60) + $dteDiffafterlunch->i;
                                    $ii = $totalminutesintvafter;
                                    //$startdatetime = 
                                    //$ii += ($datainfo->timeduration + $datainfo->intervaltime);
                                    $lunchupdate = "1";
                                }
            ...
            } else if ((count(($alldaysvalue) > 0)) && (in_array($datainfo->products, $array_of_Excutivecall))) {
                ...
                foreach ($alldaysvalue as $todaydate) {
                    ...
                    if (in_array($datainfo->products, $array_of_Excutivecall)) {
                        $counttimeslots = 0;
                        $lunchupdate = "0";
                        while ($ii <= $totalminutesintv) {
                            //echo $ii.'<pre>';print_r($totalminutesintv);
                            $strgdata['st'] = DateTime::createFromFormat("Y-m-d h:i A", $datatiming['startdate']->format("Y-m-d") . " " . $datatiming[0][1] . " " . $datatiming[0][2]);
                            $strgdata['en'] = DateTime::createFromFormat("Y-m-d h:i A", $datatiming['startdate']->format("Y-m-d") . " " . $datatiming[1][1] . " " . $datatiming[1][2]);

                            $ii += ($datainfo->timeduration);
                        }

                        //Get no of particpant list of call
                        //echo "<pre>12345";print_r($allintervals);
                        foreach ($allintervals as $slot_data) {
                            ...
                            $newtimeslotarray = array(
                                ...
                                'time_slot_end' => date('jS F, Y H:i:A', $slot_data["endtime"]->getTimestamp()),
                                'timestampend' => $slot_data["endtime"]->getTimestamp(),
                            ); //echo "<pre>1234";print_r($newtimeslotarray);
                            $this->deliveries->addTimeSlot($newtimeslotarray);
                        }
                    }
                }
    ...
    public function timeslot($oid = "", $dvyid = "", $resourceid = '') {
        ...
        //echo "<pre>";print_r($data);die;
        $array_of_call = implode(",", single_array($this->deliveries->getAllCatalystCallProducts(), 'id', 'id'));
        //$array_of_call = array(99,98,97,93,86,27,25,23,22,21,20);
        if ($this->input->post()) {
            //$diagnosedata=array();
            $postalldata = $this->input->post();
    ...
    public function updateTimeSlotpop($id = 0, $order_id = 0, $delivery_id = 0, $resource_id = 0, $user_id = 0) {
        ...
        //echo "<pre>";print_r($singleRecord);echo "<hr>";
...
//die('hhhhhhhhhh');
        ...
        foreach ($delivery['d_view'] as $k => $v) {
            ...
            //echo "<pre>12345";print_r($allInternalInfo);
    ...
    public function mailTravelRequestFormClient($oid = "", $digid = "", $user_id = "") {//die('hcgdyuchduhfc');
        ...
        $m_email = $data['order_detail']->manageremail;
        $m_name = $data['order_detail']->manager_name;
        //PDF Generate
        //echo "<pre>12345";print_r($m_email);
        //echo "<pre>12345";print_r($m_name);
        //die;
        $allDataByMode = array();
        ...
        $this->email->from($m_email, $m_name);
        $this->email->set_mailtype("html");
        // $maildata = $this->mails->getMailById(3);
        // $information = array('name'=>$data['userDetails']->name,'contact'=>$data['userDetails']->contact_no);
        //$datainfo = $this->messages->mailData($information ,$maildata->mailbody);                                 
...
//        echo "<pre>12345".$strtotime;print_r($data['time_detail']);echo "<hr>";
    ...
    function excelEmport($oid = '', $dlvid = '') {
        //die('dfsdsfsd');
        $this->load->library('email');
        $excutivecallflage = single_array($this->deliveries->excutiveCallProduct(), 'id', 'id');
        //echo '<pre>';print_r($excutivecallflage);die;

        $deliveryProduct = $this->deliveries->singleDeliveryProduct($oid, $dlvid);
        //echo '<pre>';
        //print_r($deliveryProduct->products);die;
        $this->load->model('order/orders');
        ...
        if (isset($_FILES['tmpdocuments'])) {
            ...
            if ($result >= 1) {
                ...
                //echo "<pre>1234";print_r($objPHPExcel);die;   
                ...
                foreach ($objPHPExcel as $key => $value) {
                    ...
                    $userdata['contact_no'] = $value['Contact Number'];
                    $userdata['age'] = $value['Age'];
                    $userdata['department'] = $value['Department'];
                    $userdata['designation'] = $value['Designation'];
                    $userdata['reporting_to'] = $value['Reporting to'];
                    $userdata['area_of_responsibility'] = $value['Area of Responsibility'];
                    $userdata['yrs_at_ey'] = $value['Yrs at EY'];
                    $userdata['total_experience'] = $value['Total Experience'];
                    $userdata['qualification'] = $value['Qualification'];
                    $userdata['training_attended_in_the_past'] = $value['Trainings attended in the past'];
                    $userdata['previous_employer'] = $value['Previous Employer'];
                    $userdata['email'] = $value['E-mail Address'];
                    $userdata['username'] = $value['E-mail Address'];
                    ...
                    //$userexistemail = $this->users->isUserExistEmail($id[0]);
                    //echo "<pre>".$email;print_r($userexistemail);
                    ...
                    $link = site_url('user/user_login');
                    if (in_array($deliveryProduct->products, $excutivecallflage)) {
                        //echo '<pre>';print_r($email);die;
                        //$this->email->to($email);                  
                        $this->email->to($email);
                        $this->email->from('info@maynardleighonline.in');
                        $this->email->set_mailtype("html");
                        $maildata = $this->mails->getMailById(29);
                        $information = array('name' => $name, 'USERNAME' => $email, 'PASSWORD' => '123456', 'link' => $link);
                        $datainfo = $this->messages->mailData($information, $maildata->mailbody);
                        $bodymessage = $this->messages->mailTemplate($datainfo);
                        $this->email->subject($maildata->mailsubject);
                        $this->email->subject("Maynardeleigh || Participant Import");
                        $this->email->message($bodymessage);
                        //echo "<pre>";print_r($this->email);die;
                        $status = $this->email->send();
                        $this->email->clear();
