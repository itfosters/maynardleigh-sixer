...
class Contract_accepts extends ITF_Model {

    protected $order = "order";
    //private $client="clients";
    protected $name = "users";
    private $product = "product";
    private $assign_date = "assign_date";
    ...
    public function get_diagnose($id = '') {
        $this->db->select('P.name,P.product_type');
    ...
    public function getTodayLeadershipListForSend($starttime, $endtime) {
        $this->db->select('U1.name,U1.email,AD.id,AD.order_id,AD.order_type,AD.diagnose_id,AD.start_date,AD.start_time,AD.end_date,AD.end_time,U.name as clientname,AD.status,AD.comment,AD.tstartdatetime,AD.tenddatetime');
        ...
        $this->db->join($this->name . " as U1", "CD.casting_manager=U1.id");
        //$this->db->where('AD.tstartdatetime >= ', $starttime);
        $this->db->where('AD.tenddatetime >= ', $starttime);
        $this->db->where('AD.tenddatetime <= ', $endtime);
        $this->db->where('AD.del_status', 0);
        //$this->db->where('AD.status', 1);

        //$this->db->where(array('AD.tstartdatetime >= ', $starttime, 'AD.tenddatetime <= ', $endtime, 'AD.del_status' => 0));
        $result = $this->db->get()->result();
        //echo $this->db->last_query();
        return $result;
    }
    //get all blank invoices 
    public function getTodayAllReportForInvoice($endtime) {
        $this->db->select('U1.name,U1.email,AD.id,AD.order_id,AD.order_type,AD.diagnose_id,AD.start_date,AD.start_time,AD.end_date,AD.end_time,U.name as clientname,AD.status,AD.comment,AD.tstartdatetime,AD.tenddatetime');
        $this->db->from($this->assign_date . " as AD");
        ...
        $this->db->where('AD.tenddatetime <= ', $endtime);
        $this->db->where('AD.del_status', 0);
        $this->db->where('AD.invoice_no IS NULL');
        //$this->db->where('AD.status', 1);
