...
//echo "<pre>";print_r($id);die;
    ...
    //get Order id by Assign time id
    public function getOrderIdByAssignId($AssigntimeId=0) {
        $this->db->select('A.order_id,A.manager_id,A.tstartdatetime,A.tenddatetime,A.diagnose_id,A.order_type,U.email');
        $this->db->from($this->assigndate.' A');
        $this->db->join($this->name. ' U','A.manager_id=U.id');
        $this->db->where(array('A.id'=>$AssigntimeId));
        $query=(array)$this->db->get()->row();
        return $query;
    }
