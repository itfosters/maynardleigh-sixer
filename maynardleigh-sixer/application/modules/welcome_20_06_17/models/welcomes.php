...
class Welcomes extends ITF_Model {
    ...
    public function getLeavesPopUpDetails($ids = 0) {
        $this->db->select("T2.reason as title, T1.tstartdatetime tstartdatetime,T1.tenddatetime tenddatetime ,  T1.status as status");
...
//echo "<pre>";print_r($id);die;
    ...
    public function getLeaveRequest($userId,$month=""){
        $this->db->select("T1.id diagnose_id ,T2.reason as title, date(FROM_UNIXTIME(T1.tstartdatetime)) as start,T1.tstartdatetime tstartdatetime,T1.tenddatetime tenddatetime  ,  T1.status");
        $this->db->from("itf_recource_leave_request as T1");
        $this->db->join("itf_recource_leave_reason as T2", "T1.leave_reason=T2.id");
        $this->db->where(array("T1.recource_id"=>$userId /*, "month(FROM_UNIXTIME(tstartdatetime))" =>$month */ ));
        $result = $this->db->get()->result();
       // echo $this->db->last_query(); //die;
    ...
    public function getAssignDate($userid) {
        ...
        //$this->db->where(array('manager_idS'=>$userid));
    ...
    public function getAssignDateDetails($where = array()) {
        ...
        $this->db->select('AD.id,AD.order_id,AD.order_type,AD.diagnose_id,AD.start_date,AD.start_time,AD.end_date,AD.end_time,AD.tstartdatetime,AD.tenddatetime,U.name as clientname,AD.status,AD.comment');
        $this->db->from($this->assigndate . " as AD");
