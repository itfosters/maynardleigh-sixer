...
<table style="border-collapse: collapse; width: 100%;border: 1px solid #000;font-family: Helvetica;"> <tr>
  ...
  <tr style="border: 1px solid #000; padding: 5px">
    ...
    <td style="border: 1px solid #000; padding: 5px;font-size: 14px;" colspan="4"><?php echo $order_detail->order_Id; ?><?php //echo isset($allmode['Venue'][0]->venuedetail) ? $allmode['Venue'][0]->venuedetail : 'N/A'; ?></td>
...
<?php 
               ...
               if(in_array('cab',$selection) && (isset($allmode['cab']))){
...
<tr style="border: 1px solid #000; padding: 5px">
    ...
    <td 
        <?php echo ((isset($allmode['air'][0]->food) && !empty($allmode['air'][0]->food)) || ((isset($allmode['train'][0]->food) && !empty($allmode['train'][0]->food)))) ? '':'colspan="2"';?>
        style="border: 1px solid #000; padding: 5px;height: 50px;font-size: 14px;color: rgb(9,52,86);text-align: center;"><b><?php echo isset($userDetails[0]->contact_no) ? $userDetails[0]->contact_no : ''; ?></b></td>
    <?php if((isset($allmode['air'][0]->food) && !empty($allmode['air'][0]->food)) || ((isset($allmode['train'][0]->food) && !empty($allmode['train'][0]->food)))){  ?>
    <td style="border: 1px solid #000; padding: 5px;height: 50px;font-size: 14px;color: rgb(9,52,86);text-align: center;">
        
        Meals Type (Air/Train): <?php echo (isset($allmode['air'][0]->food) && !empty($allmode['air'][0]->food)) ? ucfirst($allmode['air'][0]->food):'N/A';echo ((isset($allmode['air'][0]->food) && !empty($allmode['train'][0]->food)) ? "/".ucfirst($allmode['train'][0]->food):'/(N/A)'); ?>
...
//echo "<pre>";print_r($allmode['Hotel']);die;

  ?>

<tr style="border: 1px solid #000; padding: 5px">
    <td style="border: 1px solid #000; padding: 5px; height: 50px;font-size: 14px;color: rgb(9,52,86);"><strong>Hotel Category(minimum) in <?php echo $allmode['Hotel'][0]->city; ?></strong></td>
    <td style="border: 1px solid #000; padding: 5px;height: 50px;font-size: 14px;color: rgb(9,52,86);"><strong>Check IN Date</strong></td>
    <td style="border: 1px solid #000; padding: 5px;height: 50px;font-size: 14px;color: rgb(9,52,86);"><strong>Check OUT Date</strong></td>
...
<tr style="border: 1px solid #000; padding: 5px">
    <td style="border: 1px solid #000; padding: 5px;height: 50px;font-size: 14px;"><?php echo isset($allmode['Hotel'][0]->star_category)?$allmode['Hotel'][0]->star_category:'N/A';
    //echo !empty($allmode['Hotel'][0]->preferred_hotel) ? $allmode['Hotel'][0]->preferred_hotel:$allmode['Hotel'][0]->preferred_hotel; ?> <?php echo strstr($allmode['Hotel'][0]->star_category,'Star')?'':'Star'?></td>
    ...
    <td style="border: 1px solid #000; padding: 5px;height: 50px;font-size: 14px;">Single occupancy, Minimum <?php echo isset($allmode['Hotel'][0]->star_category)?$allmode['Hotel'][0]->star_category:'N/A';?> Star.<?php if(!($allmode['Hotel'][0]->breakfast==0 && $allmode['Hotel'][0]->meals==0)){?> Please include the <?php echo $allmode['Hotel'][0]->breakfast==1?" breakfast  ":'' ; ?>  <?php echo ($allmode['Hotel'][0]->breakfast==1 && $allmode['Hotel'][0]->meals==1)?' & ':'';  ?>    <?php echo $allmode['Hotel'][0]->meals==1?" meals  ":'' ; ?> in the package during the hotel confirmation<?php }?></td> 
</tr>
 <?php } ?>
