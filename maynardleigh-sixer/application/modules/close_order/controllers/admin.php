...
class Admin extends ITFS_Controller {
    ...
    public function index() 
    {
        ...
        if(isset($_POST["q"])) {
            $this->session->set_userdata(array("CLOSEDSEARCHFIELD"=>$this->input->post()));
        } 

        $searchfield = $this->session->userdata("CLOSEDSEARCHFIELD");
        ...
        // End of Filter Data
        $totaldata=$this->close_orders->totalData($conditions);
       //echo "##<pre>";print_r($totaldata);die;
            ...
            $data['alldata']=$this->close_orders->findUser($conditions,$config["per_page"],$page);
            ...
            $this->template->headingtitle="CLOSED ORDER LISTING";
            //$this->template->set_breadcrumb("Add Order",site_url("admin/order/add_order"));
            $this->template->set_breadcrumb("CLOSED ORDER LISTING","");
