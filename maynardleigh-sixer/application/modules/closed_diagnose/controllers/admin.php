...
//        $searchfield = $this->session->userdata("SEARCHFIELD");
        ...
        if(isset($_POST["q"])) {
            $this->session->set_userdata(array("CLOSEDDIAGNOSESEARCHFIELD"=>$this->input->post()));
        } 

        $searchfield = $this->session->userdata("CLOSEDDIAGNOSESEARCHFIELD");
        if(count($searchfield)>0)
        $data["frm_data"] = $searchfield;
        ...
        // End of Filter Data
          ...
          //echo "<pre>";print_r($data);die;
            $this->template->headingtitle="CLOSED DIAGNOSE LISTING";
            $this->template->set_breadcrumb("CLOSED ORDER LISTING",site_url("admin/close_order"));
             $this->template->set_breadcrumb("CLOSE DIAGNOSE LISTING ","");
