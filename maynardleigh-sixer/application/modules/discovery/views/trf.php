    ...
    <div class="row">
        ...
        <table class="table">
          ...
          <tr>      
            ...
            <a id="clientmails" class='travel-pop' title='Send To Client' href='<?php echo site_url('admin/discovery/mailTravelRequestFormNewClient/'.$frmdata['odered_id'].'/'.$frmdata['diagnose_id']); ?>'><i class='fa fa-envelope btn '></i></a>
<!--                <a target="_blank" id="vendermails" title="Send To Resource" href="<?php echo site_url('admin/discovery/mailVenderRequestForm/'.$frmdata['odered_id'].'/'.$frmdata['diagnose_id']); ?>"><i class="fa fa-envelope-o btn"></i></a> -->
                <a id="vendermailspreview" title="Send To Resource" href="<?php echo site_url('admin/discovery/newvendermailslinkspreview/'.$frmdata['odered_id'].'/'.$frmdata['diagnose_id']); ?>"><i class="fa fa-envelope-o btn"></i></a>
            </th> 
...
<script type="text/javascript">
     //var resource_details_temp = "<tr><td>${mode}</td><td>${journey_date}</td><td>${return_date}</td><td>${description}</td><td><a class='fa fa-trash resource_act' href='#itf' rel='${id}'></a>&nbsp;&nbsp;&nbsp;&nbsp;<a class='fa fa-pencil-square-o travel-pop btnwd' href='<?php echo site_url('admin/discovery/trfPopUp/${order_id}/${diagnose_id}/${user_id}/${id}'); ?>' class='travel-pop'></a>&nbsp;{{if mode!='Venue'}}&nbsp;&nbsp<a class='fa fa-eye btnwd' href='<?php echo site_url('admin/discovery/trfAllDetails/${order_id}/${diagnose_id}/${user_id}/${id}'); ?>' class='travel-pop'></a>&nbsp;&nbsp;{{/if}}<a  class='travel-pop' title='Send To Travel Agent' href='<?php echo site_url('admin/discovery/mailTravelRequestFormNew/'.$frmdata['odered_id'].'/'.$frmdata['diagnose_id'].'/${user_id}'.'/${id}'); ?>'><i class='fa fa-envelope btn '></i></a></td></tr>";
     var resource_details_temp = "<tr> <td>${mode}</td><td>${journey_date}</td><td>${return_date}</td><td>${description}</td><td colspan='2' width='15%'> <a class='fa fa-trash resource_act' href='#itf' rel='${id}'></a>&nbsp;&nbsp;&nbsp;&nbsp; <a class='fa fa-pencil-square-o travel-pop btnwd' href='<?php echo site_url('admin/discovery/trfPopUp/${order_id}/${diagnose_id}/${user_id}/${id}'); ?>' class='travel-pop'></a>&nbsp;{{if mode!='Venue'}}&nbsp;&nbsp <a class='fa fa-eye btnwd' href='<?php echo site_url('admin/discovery/trfAllDetails/${order_id}/${diagnose_id}/${user_id}/${id}'); ?>' class='travel-pop'></a>&nbsp;&nbsp;{{/if}}{{if emailflag==1}}<a class='travel-pop' title='Send To Travel Agent' href='<?php echo site_url('admin/discovery/mailTravelRequestFormNew/' . $frmdata['odered_id'] . '/' . $frmdata['diagnose_id'] . '/${user_id}' . '/${id}'); ?>'><i class='fa fa-envelope btn '></i> </a>{{/if}}</td></tr>";
   ...
   var newmailslinks=ADMINURL+'discovery/mailTravelRequestForm/'+alinks+'/'+ids;
    ...
    var newvendermailslinkspreview=ADMINURL+'discovery/mailVenderRequestFormPreview/'+alinks+'/'+ids;
    $('#vendermailspreview').attr("href", newvendermailslinkspreview);
    
    var newvendermailslinksclient=ADMINURL+'discovery/mailTravelRequestFormNewClient/'+alinks+'/'+ids;
  ...
  $(document).ready(function (){
   ...
   // $('.selectpicker').on('change', function(){
      ...
      if(confirm("Do you want to Delete?")){
        ...
        $.ajax({
          url:"<?php echo site_url('admin/discovery/deleteResources'); ?>",
          data:{id:id},
          method:"POST",
          success:function(res){
            if(res.results='true'){
