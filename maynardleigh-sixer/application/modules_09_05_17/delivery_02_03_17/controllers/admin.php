       ...
       // echo "<pre>";print_r($deleteIdsarray);die;
        foreach($deleteIdsarray as $resourceid){
            ...
            $information = array('name'=> ucfirst($singeInfo->resourcename),'client'=>ucfirst($singeInfo->clientname),'job'=>$singeInfo->proname,'subproduct'=>$singeInfo->name,'location'=>$singeInfo->location,
                                'date'=>$datevalue,
                                'time'=>date("h:i:s A",$singeInfo->tstartdatetime)."-".date("h:i:s A",$singeInfo->tenddatetime),
                                'link'=>$link);
    ...
    public function calender($oid="",$dvyid="",$resourceid='')
    {
        ...
        if($this->input->post())
        {
            ...
            while($ii<=$totalminutesintv)
            {
                ...
                $lunchtimedata['st']=DateTime::createFromFormat("Y-m-d H:i A",$datatiming['startdate']->format("Y-m-d")." ".$datainfo->lunchstarttime);
                $lunchtimedata['en']=DateTime::createFromFormat("Y-m-d H:i A",$datatiming['startdate']->format("Y-m-d")." ".$datainfo->lunchendtime);
                ...
                /*echo "<pre>12345";print_r($datatiming);
                echo "<pre>12345";print_r($strgdata);
                echo "<pre>12345";print_r($lunchtimedata);die;*/
                if(
