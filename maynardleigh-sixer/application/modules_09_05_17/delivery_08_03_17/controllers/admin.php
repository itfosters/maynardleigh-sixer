...
//                    )  
                    ...
                    $allintervals[]=array("start"=>$sttime,'endtime'=>$endtime,'lunchflage'=>1);
    ...
    {//die('hcgdyuchduhfc');
        ...
        if($this->input->post()){
            ...
            $information = array(
                ...
                'no_of_participant'=>$allPost['no_of_participant'],
                'vms'=>$allPost['vms'],
            ...
            $datainfo = $this->messages->mailData($information ,$maildata->mailbody);                                 
            $bodymessage = $this->messages->mailTemplate($datainfo);
            //echo "<pre>123";print_r($maildata);die;
            $this->email->subject($maildata->mailsubject);
            $this->email->message($bodymessage); 
            //echo "<pre>";print_r($this->email);die;
            if(!empty($trainticket)){
                $this->email->attach($trainticket);
                //unlink($trainticket);
            }
            if(!empty($airticket)){
                $this->email->attach($airticket);
                //unlink($airticket);
            }
            if(!empty($cabticket)){
