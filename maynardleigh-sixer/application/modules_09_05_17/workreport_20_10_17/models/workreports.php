...
$this->db->select('AD.id,AD.order_id,AD.order_type,AD.diagnose_id,AD.start_date,AD.start_time,AD.end_date,AD.end_time,U.name as clientname,AD.status');
		...
		$this->db->group_by('AD.diagnose_id');
		$query=$this->db->get()->result();
		//echo"<pre>@@";print_r($this->db->last_query()); die;
...
$mid=$where['casting_id'];
$this->db->select('AD.id,AD.order_id,AD.order_type,AD.diagnose_id,AD.start_date,AD.entry_date,AD.start_time,AD.end_date,AD.end_time,U.name as clientname,AD.status');
		$this->db->from($this->casting." as C");
...
public function getMoreInfoForResource($order_id,$order_type){
	//echo $this->tableArray[$order_type]."<hr>";
	 $this->db->select('O.location,P.name,P.weight,S.name as subname',false);
