...
<label><h3></h3></label>
<?php
echo form_open('admin/workreportclient/downloadExcel');
     ...
     $('.sdate').text("Please Select Start Date");
        }else if(cal2=='')
        {
         $('.edate').text("Please Select End Date");
                ...
                $('.udate').text("");
       $.ajax({
                url: "<?=site_url()?>workreportclient/admin/showAllDates",
                type: "POST",
                ...
                success: function (emp_list) {
                   // console.log(emp_list);
                    $(".table").empty();
                    var headertext = "<tr><th>Order ID</th><th>Job</th><th>Subproducts</th><th>Location</th><th>Start Date</th><th>End Date</th><th>Status</th></tr>";
                    ...
                    else if (item.status=='1')
                    {
                        ...
                        status='Rejected';
                        txt = "<tr><td>"+item.order_id+"</td><td>"+pname+"</td>"+
                         "<td>"+spname+"</td><td>"+location+"</td>"+
                         "<td>"+item.start_date+"</td><td>"+item.end_date+"</td><td class='status'>"+status+"</td></tr>";
                        //alert(txt);
